...
export default function ResetPasswordPage() {
  ...
  // Po kliknięciu w link e-mail callback wymienia code na sesję recovery.
  // Tutaj weryfikujemy że taką sesję faktycznie mamy.
  useEffect(() => {
  ...
  if (hasSession === null) {
    return <div className="min-h-screen flex items-center justify-center text-sm text-zinc-500">Ładowanie…</div>;
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-zinc-50 dark:bg-zinc-950 p-4">
        <div className="w-full max-w-sm bg-white dark:bg-zinc-900 rounded-2xl shadow-sm border border-zinc-200 dark:border-zinc-800 p-8 text-center">
          ...
          <h1 className="text-xl font-bold mb-2">Link wygasł</h1>
          <p className="text-sm text-zinc-500 mb-6">
            Link do resetu hasła jest nieprawidłowy lub stracił ważność. Spróbuj jeszcze raz.
          </p>
          <Link href="/forgot-password" className="text-blue-500 hover:underline text-sm">Wyślij nowy link</Link>
        </div>
      </div>
    );
  }
      ...
      <div className="w-full max-w-sm bg-white dark:bg-zinc-900 rounded-2xl shadow-sm border border-zinc-200 dark:border-zinc-800 p-8">
        ...
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          <input
            ...
            className="w-full px-4 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
          {password2 && !match && <p className="text-sm text-red-500 text-center">Hasła nie są identyczne lub za krótkie.</p>}
